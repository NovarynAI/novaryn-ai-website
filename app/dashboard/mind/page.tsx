  ...
  async function submitAnswer() {
    ...
    try {
      const res = await fetch("/api/mind", {
        ...
        body: JSON.stringify({ category: exercise.category, question: exercise.question, answer: exercise.answer, userAnswer }),
      });
      const data = await res.json();
      setResult(data);
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <div className="max-w-xl mx-auto px-6 py-16">
        <div className="text-center mb-12">
          ...
          <h1 className="text-5xl font-bold mb-3 bg-gradient-to-r from-purple-400 via-pink-400 to-indigo-400 bg-clip-text text-transparent">Novaryn Mind</h1>
          ...
          <div className="space-y-4">
            ...
            {!result && (
              <div className="space-y-3">
                <textarea value={userAnswer} onChange={(e) => setUserAnswer(e.target.value)} placeholder="Cevabini buraya yaz..." className="w-full bg-[#111] border border-white/10 rounded-2xl px-6 py-4 text-white placeholder-white/20 resize-none focus:outline-none focus:border-purple-400/50 transition" rows={4} />
                <div className="flex gap-3">
                  <button onClick={() => setShowHint(!showHint)} className="flex-1 border border-white/10 rounded-2xl py-3 text-sm text-white/40 hover:text-white hover:border-white/20 transition">
                    {showHint ? "Ipucunu gizle" : "Ipucu goster"}
                  </button>
                  <button onClick={submitAnswer} disabled={!userAnswer.trim() || evaluating} className={"flex-1 py-3 rounded-2xl text-sm font-bold transition bg-gradient-to-r " + config.color + " text-white disabled:opacity-30 disabled:cursor-not-allowed"}>
              ...
              <div className="space-y-4">
                <div className={"bg-[#111] border rounded-2xl px-8 py-6 " + (result.correct ? "border-green-400/30" : "border-orange-400/30")}>
                  <div className="flex items-center justify-between mb-4">
                    <p className={"text-2xl font-bold " + (result.correct ? "text-green-400" : "text-orange-400")}>{result.correct ? "Dogru!" : "Yakin!"}</p>
