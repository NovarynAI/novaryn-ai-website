...
type Exercise = { name: string; muscle: string; sets: number; reps: string; instruction: string; };
type Day = { day: string; type: "workout" | "rest"; focus: string; exercises: Exercise[]; };
type Plan = { split: string; days: Day[]; };
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <div className="max-w-xl mx-auto px-6 py-16">
        <div className="text-center mb-12">
          ...
          <h1 className="text-5xl font-bold mb-3 bg-gradient-to-r from-green-400 to-emerald-400 bg-clip-text text-transparent">Novaryn Fit</h1>
          ...
          <div className="space-y-8">
            <div>
              ...
              <div className="grid grid-cols-4 gap-3">
                {dayOptions.map((d) => (
                  <button key={d} onClick={() => setDays((prev) => (prev === d ? null : d))} className={"rounded-2xl py-5 border transition text-center " + (days === d ? "border-green-400 bg-green-400/10" : "border-white/10 bg-[#111] hover:border-white/20")}>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {([{ id: "home" as const, label: "Ev", desc: "Ekipmansiz" }, { id: "gym" as const, label: "Spor Salonu", desc: "Ekipmanli" }]).map((l) => (
                  <button key={l.id} onClick={() => toggleLocation(l.id)} className={"rounded-2xl px-4 py-4 border transition text-left " + (location === l.id ? "border-green-400 bg-green-400/10" : "border-white/10 bg-[#111] hover:border-white/20")}>
            ...
            <div>
              ...
              <div className="flex gap-3">
                <button onClick={() => toggleMuscle("full")} className={"rounded-2xl px-4 py-4 border transition text-left min-w-[110px] " + (isFull ? "border-green-400 bg-green-400/10" : "border-white/10 bg-[#111] hover:border-white/20")}>
                  <p className="text-2xl mb-1">🏃</p>
                  <p className={"font-semibold text-sm " + (isFull ? "text-green-400" : "")}>Tum Vucut</p>
                </button>
                <div className="grid grid-cols-2 gap-2 flex-1">
                  {muscleGroups.map((m) => (
                    <button key={m.id} onClick={() => toggleMuscle(m.id)} className={"rounded-xl px-3 py-2 border transition text-left text-sm " + (selectedMuscles.includes(m.id) ? "border-green-400 bg-green-400/10 text-green-400" : "border-white/10 bg-[#111] hover:border-white/20 text-white/60")}>{m.label}</button>
                  ))}
                </div>
              </div>
            </div>
            <button onClick={getPlan} disabled={!isReady || loading} className="w-full bg-gradient-to-r from-green-400 to-emerald-500 text-black py-4 rounded-2xl font-bold text-base hover:scale-[1.02] transition-transform disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:scale-100">
        ...
        {plan && (
          <div className="space-y-6">
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
              {plan.days.map((d, i) => (
                <button key={i} onClick={() => setActiveDay(i)} className={"rounded-xl px-3 py-2 border transition text-center min-w-[80px] " + (activeDay === i ? (d.type === "rest" ? "border-white/30 bg-white/5" : "border-green-400 bg-green-400/10") : "border-white/10 bg-[#111] hover:border-white/20")}>
            ...
            {plan.days[activeDay] && (
              <div className="space-y-3">
                <div className="flex items-center justify-between mb-4">
                  ...
                  {plan.days[activeDay].type === "rest" && <span className="text-xs text-white/30 border border-white/10 rounded-full px-3 py-1">Dinlenme</span>}
                </div>
                {plan.days[activeDay].type === "rest" ? (
                  ...
                  plan.days[activeDay].exercises.map((ex, i) => (
                    <div key={i} className="bg-[#111] border border-white/5 rounded-2xl px-6 py-5">
                      ...
                      <a href={"https://www.youtube.com/results?search_query=" + encodeURIComponent(ex.name + " how to exercise")} target="_blank" rel="noopener noreferrer" className="text-xs text-red-400/70 hover:text-red-400 transition">Nasil yapilir?</a>
                    </div>
                  ))
                )}
              </div>
            )}
            <button onClick={() => { setPlan(null); setSelectedMuscles([]); setDays(null); setLocation(null); }} className="w-full border border-white/10 rounded-2xl py-4 text-sm text-white/40 hover:text-white hover:border-white/20 transition mt-4">Yeni plan olustur</button>
          </div>
        )}
      </div>
    </div>
